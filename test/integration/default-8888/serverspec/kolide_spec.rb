...
describe file('/var/log/syslog'), :if => os[:family] == 'debian' || os[:family] == 'ubuntu' do
  ...
  its(:content) { should match /kolide\[[0-9]+\]: {"address":"0.0.0.0:8888","msg":"listening","transport":/ }
...
describe file('/var/log/messages'), :if => os[:family] == 'redhat' do
  ...
  its(:content) { should match /kolide: {"address":"0.0.0.0:8888","msg":"listening","transport":/ }
  its(:content) { should_not match /kolide: .*permission denied.*/ }
end

# Note: probably requires fleet initial configuration...
describe command('curl -vk https://localhost:8888/healthz') do
  its(:stdout) { should match /^$/ }
#  its(:stderr) { should match /HTTP\/1.1 200 OK/ }
  its(:exit_status) { should eq 0 }
end
describe command('curl -vk -X POST https://localhost:8888/api/v1/osquery/enroll') do
